 ...
 *     default, so sanitizeField needs no special-case handling for them.
 *     Do NOT call sanitizeField on `ocr_text` values that intentionally
 *     carry `{{`/`{%` syntax — the Fisqua passthrough adapter skips
 *     sanitisation entirely because Fisqua is a trusted first-party source.
 *
