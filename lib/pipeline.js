...
// ---------------------------------------------------------------------------
// runMets — generate METS documents (independent of the Hugo build)
// ---------------------------------------------------------------------------

/**
 * Run scripts/generate-mets.js to write one METS document per description to
 * METS_DIR (default INSTANCE_ROOT/public/mets). Independent of `runBuild` — it
 * reads exports/ + hugo.toml, not the rendered site, so it can run on its own
 * step/cadence and target a separate host. No theme copy, no Hugo language
 * derivation. DATA_DIR / METS_DIR pass through from the environment.
 *
 * Deliberately does NOT run the full input validator: METS depends only on
 * descriptions.json + repositories.json (generate-mets.js fails fast and
 * clearly if descriptions.json is absent), and the command is meant to be
 * runnable standalone in a deploy step without re-validating the whole export.
 *
 * @returns {Promise<void>}
 */
async function runMets() {
  const { INSTANCE_ROOT, ENGINE_ROOT } = resolveRoots();
  const script = path.join(ENGINE_ROOT, 'scripts', 'generate-mets.js');
  const env = {
    ...process.env,
    INSTANCE_ROOT,
    ENGINE_ROOT,
  };
  await spawnAsync('node', [script], { cwd: INSTANCE_ROOT, env });
}

module.exports = { runBuild, runDev, runMets };
