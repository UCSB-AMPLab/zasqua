 ...
 *
 * Provides the three pipeline entry points that the Zasqua CLI dispatches
 * to: `runBuild`, `runFetch`, and `runDev`. Each wrapper sets the correct
 ...
 *     ENGINE_ROOT pointing at the engine package. Forwards SKIP_DOWNLOAD
 *     and DEV_LIMIT from the caller's environment; build.sh already
 *     honours both. Resolves when the child exits 0; rejects on nonzero.
 *
 *   runFetch(opts)
 *     Runs Stage 1 (B2 data download) only. Spawns build.sh with
 *     FETCH_ONLY=1, which causes build.sh to exit immediately after the
 *     Stage 1 download block. Requires B2_APPLICATION_KEY_ID and
 *     B2_APPLICATION_KEY in the environment (unchanged from today).
 *
 *   runDev(opts)
 ...
 *   FETCH_ONLY     — (internal) set by runFetch to exit after Stage 1
...
/**
 * Copy base theme then run the full 7-stage build.sh pipeline.
 *
 ...
 *
 * Forwards SKIP_DOWNLOAD and DEV_LIMIT from the environment; build.sh
 * already honours both flags. Does not forward FETCH_ONLY (that is an
 * internal runFetch flag).
 *
 * @param {object}  [opts]
...
// ---------------------------------------------------------------------------
// runFetch — Stage 1 (B2 data download) only
// ---------------------------------------------------------------------------

/**
 * Run Stage 1 (B2 data download) only, by setting FETCH_ONLY=1.
 *
 * build.sh exits immediately after Stage 1 completes when FETCH_ONLY is
 * set (see the FETCH_ONLY guard in build.sh). Requires
 * B2_APPLICATION_KEY_ID and B2_APPLICATION_KEY in the environment;
 * these are forwarded unchanged from process.env.
 *
 * @returns {Promise<void>}
 */
...
module.exports = { runBuild, runFetch, runDev };
