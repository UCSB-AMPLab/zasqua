 ...
 *   `cpSync` with `force: true` copies source files into the destination
 *   but cannot delete anything already there. A file removed from the
 *   engine source (e.g. a per-section content adapter such as
 *   entidades/_content.gotmpl) would otherwise persist in the instance
 *   across builds — producing spurious pages like
 *   /entidades/entidades/ or /lugares/lugares/. Removing the destination
 *   directory first ensures the instance always reflects the engine
 *   source exactly, with no stale artefacts.
 *
...
function stampEngineVersion(engineRoot, dest) {
  ...
  try {
    ...
    // package.json is written by npm install; missing/unreadable here means
    // the engine install itself is broken. Leave the committed fallback
    // engine.yaml in place rather than fail the build over it.
    return;
  }
  if (!version) {
    return;
