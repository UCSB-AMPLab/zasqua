...
function extractIiifManifest(daos, quirks) {
  ...
  for (const dao of daos) {
    ...
    // PARSER_OPTS sets attributeNamePrefix: '@_' with no removeNSPrefix, so
    // '@_xlink:href' is the only namespaced form this parser can produce;
    // '@_xlink_href' is kept only as a defensive no-op — no known source emits it.
...
async function run({ src, stagingDir, standard, instanceRoot }) {
  ...
  const daoCount = entityLinks.length; // unused: superseded by daoDescCount below
